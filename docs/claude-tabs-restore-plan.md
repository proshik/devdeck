...
2. **Перечисление работает прямо сейчас** и отдаёт ровно то, что нужно:
   `TAB 4 | ✳ fix-own-memory-group-chat | /Users/me/work/project/myapp`.
...
4. **Из процесса id сессии не достать.** `lsof` отдаёт `cwd`, но транскрипт claude открытым
   не держит; к тому же у пользователя три процесса сидят в одном `myapp` — cwd как ключ не годится.
...
2. **cwd → каталог проекта**: слаг = путь, в котором `/` заменён на `-` (`/Users/me/work/…/sideapp`
   → `-Users-me-work-…-sideapp`). Если каталога по слагу нет — фолбэк: перебрать
