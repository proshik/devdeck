...
Sharing Mac: `macbook-pro-home`, `192.168.50.117:9999`, VPN egress `198.51.100.7`.

| Check | Result | Meaning |
|---|---|---|
| `route -n get 192.168.50.117` | `interface: en0` | the VPN did NOT capture the LAN subnet |
| `nc -vz 192.168.50.117 9999` | `succeeded!` | unicast TCP to the proxy works |
| `dns-sd -B _devdeck-proxy._tcp` | no `Add` lines at all | multicast (224.0.0.251) is filtered |
| `curl --proxy http://192.168.50.117:9999 https://api.ipify.org` | `198.51.100.7` | the proxy really egresses through the VPN |
...
cache outlives the Wi-Fi it was learned on, so on the next network `192.168.50.117:9999` addresses
