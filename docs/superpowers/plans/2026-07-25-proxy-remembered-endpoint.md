...
```swift
    func testRememberedEndpointRoundTrips() throws {
        ...
        config.settings.activeProxyHost = "192.168.50.117"
        ...
        XCTAssertEqual(decoded.settings.activeProxyHost, "192.168.50.117")
...
final class CommandStoreProxyEndpointTests: XCTestCase {
    ...
    func testRememberEndpointPersists() throws {
        ...
        store.rememberActiveProxyEndpoint(host: "192.168.50.117", port: 9999, authRequired: false)

        XCTAssertEqual(store.config.settings.activeProxyHost, "192.168.50.117")
        XCTAssertEqual(store.config.settings.activeProxyPort, 9999)

        let fresh = CommandStore(configURL: url)
        fresh.reload()
        XCTAssertEqual(fresh.config.settings.activeProxyHost, "192.168.50.117",
    ...
    func testChangedEndpointIsPersisted() {
        ...
        store.rememberActiveProxyEndpoint(host: "192.168.50.200", port: 8888, authRequired: true)

        XCTAssertEqual(store.config.settings.activeProxyHost, "192.168.50.200")
    ...
    func testDeselectingTheProxyClearsTheEndpoint() {
        ...
        store.rememberActiveProxyEndpoint(host: "192.168.50.117", port: 9999, authRequired: true)
...
```swift
    func testRoutesThroughARememberedEndpoint() async {
        let (manager, store, fake) = makeManager()
        manager.startDiscovery()
        fake.emit([DiscoveredProxy(name: "personal-mac", host: "192.168.50.117", port: 9999,
        ...
        XCTAssertEqual(manager.routing(for: flagged()),
                       .routed(env: proxyEnv(host: "192.168.50.117", port: 9999, user: nil, pass: nil)))
        _ = store
    }
...
On the personal Mac (sharing, `192.168.50.117:9999`): leave DevDeck running.
On the work Mac:
1. With the corporate VPN **off**, confirm `macbook-pro-home` is discovered and selected active.
...
4. Run the flagged `proxy check` command → expect the 8 proxy variables and `198.51.100.7`.
