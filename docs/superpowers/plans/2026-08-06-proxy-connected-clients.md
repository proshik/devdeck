...
final class ProxyClientLogTests: XCTestCase {
    ...
    private let opened = """
        {"client":"192.168.50.42:55904","handler":"http","kind":"handler","level":"info",\
        "listener":"tcp","local":"192.168.50.5:9999","msg":"192.168.50.42:55904 <> 192.168.50.5:9999",\
        "network":"","remote":"192.168.50.42:55904","service":"devdeck-proxy","sid":"d9q51fl1837ipckfutrg",\
        "time":"2026-08-06T12:15:42.246+03:00"}
        """

    private let closed = """
        {"client":"192.168.50.42:55904","duration":435225125,"handler":"http","inputBytes":706,\
        "kind":"handler","level":"info","listener":"tcp","local":"192.168.50.5:9999",\
        "msg":"192.168.50.42:55904 >< 192.168.50.5:9999","network":"","outputBytes":3105,\
        "remote":"192.168.50.42:55904","service":"devdeck-proxy","sid":"d9q51fl1837ipckfutrg",\
    ...
    private let dialed = """
        {"client":"192.168.50.42:55904","clientID":"","dst":"104.26.13.205:443","handler":"http",\
        "host":"api.ipify.org:443","kind":"handler","level":"info","listener":"tcp",\
        "local":"192.168.50.5:9999","msg":"192.168.50.42:55904 <-> api.ipify.org:443","network":"tcp",\
        "remote":"192.168.50.42:55904","service":"devdeck-proxy","sid":"d9q51fl1837ipckfutrg",\
        "time":"2026-08-06T12:15:42.384+03:00"}
        """

    private let dialClosed = """
        {"client":"192.168.50.42:55904","clientID":"","dst":"104.26.13.205:443","duration":297735250,\
    ...
    func testSessionOpenIsRecognized() {
        XCTAssertEqual(parseGostLogLine(opened),
                       .sessionOpened(client: "192.168.50.42:55904", sid: "d9q51fl1837ipckfutrg"))
    }
    ...
    func testClientIPDropsTheEphemeralPort() {
        XCTAssertEqual(proxyClientIP("192.168.50.42:55904"), "192.168.50.42")
    }
    ...
    func testClientIPRejectsGarbage() {
        XCTAssertNil(proxyClientIP("192.168.50.42"), "no port → not a gost client string")
...
///
///     {"client":"192.168.50.42:55904","msg":"192.168.50.42:55904 <> 192.168.50.5:9999","sid":"d9q…"}
///     {"client":"192.168.50.42:55904","msg":"192.168.50.42:55904 >< 192.168.50.5:9999","sid":"d9q…"}
///
...
final class ProxyClientMonitorTests: XCTestCase {
    ...
    override func setUp() {
        super.setUp()
        clock = TestClock()
        naming = FakeProxyClientNaming(names: ["192.168.50.42": "macbook-vasya"])
    }
    ...
    private func openLine(_ client: String, sid: String) -> String {
        """
        {"client":"\(client)","kind":"handler","level":"info","local":"192.168.50.5:9999",\
        "msg":"\(client) <> 192.168.50.5:9999","service":"devdeck-proxy","sid":"\(sid)"}
        """
    }
    ...
    func testTwoSessionsFromOneMachineAreOneClient() {
        ...
        monitor.ingest(openLine("192.168.50.42:1000", sid: "a"))
        monitor.ingest(openLine("192.168.50.42:1001", sid: "b"))
        monitor.publishNow()

        XCTAssertEqual(monitor.clients.count, 1)
        XCTAssertEqual(monitor.clients.first?.ip, "192.168.50.42")
    ...
    func testClosingOneSessionLeavesTheOther() {
        ...
        monitor.ingest(openLine("192.168.50.42:1000", sid: "a"))
        monitor.ingest(openLine("192.168.50.42:1001", sid: "b"))
        monitor.ingest(closeLine("192.168.50.42:1000", sid: "a"))
        monitor.publishNow()
    ...
    func testTwoMachinesAreTwoClients() {
        let monitor = makeMonitor()
        monitor.ingest(openLine("192.168.50.42:1000", sid: "a"))
        monitor.ingest(openLine("192.168.50.77:2000", sid: "b"))
        monitor.publishNow()
    ...
    func testAResolvedNameReachesTheClient() async {
        let monitor = makeMonitor()
        monitor.ingest(openLine("192.168.50.42:1000", sid: "a"))
        await sleepUntil({ self.naming.calls == ["192.168.50.42"] },
    ...
    func testAnUnresolvedMachineIsShownByItsAddress() async {
        let monitor = makeMonitor()
        monitor.ingest(openLine("192.168.50.99:1000", sid: "a"))
        await sleepUntil({ self.naming.calls == ["192.168.50.99"] },
                         message: "the resolver was never asked")
        monitor.publishNow()

        XCTAssertNil(monitor.clients.first?.hostname)
        XCTAssertEqual(monitor.clients.first?.displayName, "192.168.50.99")
    }
    ...
    func testAFailedLookupIsNotRetriedOnEveryRequest() async {
        ...
        monitor.ingest(openLine("192.168.50.99:1001", sid: "b"))
        clock.advance(60)
        monitor.ingest(openLine("192.168.50.99:1002", sid: "c"))
        ...
        monitor.ingest(openLine("192.168.50.99:1003", sid: "d"))
    ...
    func testActiveMachinesSortAboveRetiredOnes() {
        ...
        XCTAssertEqual(monitor.clients.map(\.ip), ["192.168.50.77", "192.168.50.42"])
    }
}
```
...
final class ProxyClientWiringTests: XCTestCase {
    ...
    func testOnlyTheListenersOwnOutputCounts() {
        ...
        proxy.ingestDaemonOutput(ProxyShare.daemonID, openLine("192.168.50.42:1000", sid: "a"))
        proxy.ingestDaemonOutput(UUID(), openLine("192.168.50.77:2000", sid: "b"))
        monitor.publishNow()

        XCTAssertEqual(proxy.proxyClients.map(\.ip), ["192.168.50.42"],
