...
final class ProxyEnvFileTests: XCTestCase {
    ...
    func testContentsCarryTheTwoKeysTheHelperReads() {
        let text = proxyEnvFileContents(url: "http://192.168.50.117:9999", lanPrefix: "192.168.50")

        XCTAssertTrue(text.contains("DEVDECK_PROXY_URL=http://192.168.50.117:9999"))
        XCTAssertTrue(text.contains("DEVDECK_PROXY_LAN=192.168.50"))
    }
    ...
    func testURLWithoutCredentials() {
        XCTAssertEqual(proxyURL(host: "192.168.50.117", port: 9999, user: nil, pass: nil),
                       "http://192.168.50.117:9999")
...
final class ProxyManagerEnvFileTests: XCTestCase {
    ...
    private func makeRig(lanIP: String? = "192.168.50.84",
    ...
    private func proxy(_ name: String, host: String = "192.168.50.117", port: Int = 9999,
    ...
    func testWritesTheFileWhenAProxyResolves() async {
        ...
        XCTAssertTrue(contents.contains("DEVDECK_PROXY_URL=http://192.168.50.117:9999"))
        XCTAssertTrue(contents.contains("DEVDECK_PROXY_LAN=192.168.50"),
    ...
    func testAChangedEndpointDoesRewriteTheFile() async {
        ...
        fake.emit([proxy("personal-mac", host: "192.168.50.200")])
        await yieldUntil { manager.activeProxy?.host == "192.168.50.200" }

        XCTAssertTrue(try! XCTUnwrap(envFile.contents).contains("http://192.168.50.200:9999"),
    ...
    func testCredentialsAreIncludedWhenTheProxyRequiresThem() async {
        ...
        XCTAssertTrue(try! XCTUnwrap(envFile.contents).contains("http://dev:s3cret@192.168.50.117:9999"))
    }
...
final class ProxyShellHelperTests: XCTestCase {
    ...
    func testRefusesWhenTheEnvFileIsIncomplete() throws {
        try writeEnvFile("DEVDECK_PROXY_URL=http://192.168.50.117:9999\n")
