...
final class ProxyManagerRoutingResolutionTests: XCTestCase {
    ...
    private func makeManager(credentials: FakeProxyCredentialStore = FakeProxyCredentialStore(),
                             lanIP: @escaping () -> String? = { "192.168.50.10" })
    ...
    func testRememberedEndpointResolvesBackOnTheSameLAN() async {
        ...
        let (manager, _, fake) = makeManagerKeepingStore(lanIP: { "192.168.50.10" })
        manager.startDiscovery()
        fake.emit([DiscoveredProxy(name: "personal-mac", host: "192.168.50.117", port: 9999,
                                  authRequired: false, exitIP: nil, proto: "http+socks", schema: 1)])
        await yieldUntil { manager.discovered.count == 1 }
        ...
        XCTAssertEqual(manager.activeProxy?.host, "192.168.50.117")
        XCTAssertEqual(manager.routing(for: flagged()),
                       .routed(env: proxyEnv(host: "192.168.50.117", port: 9999, user: nil, pass: nil)))
    }

    func testRememberedEndpointIsIgnoredOnADifferentLAN() async {
        // The laptop moves to another network. 192.168.50.117 there is a stranger's machine, and
        // with auth cached we would hand it the proxy password without the user doing anything.
        var lanIP: String? = "192.168.50.10"
    ...
    func testHandEditedNonsenseEndpointIsNotUsed() {
        ...
        // config.json is explicitly hand-editable — "http://192.168.50.117:0" is not a proxy.
        store.rememberActiveProxyEndpoint(host: "192.168.50.117", port: 0, authRequired: false,
                                          lanPrefix: "192.168.50")
