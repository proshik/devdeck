...
final class ProxyClientMonitorTests: XCTestCase {
    ...
    func testAStaleResolveDoesNotNameTheEntryCreatedAfterAClear() async {
        ...
        monitor.clear()
        monitor.ingest(openLine("192.168.50.42:2000", sid: "b"))
        await sleepUntil({ self.naming.calls.count == 2 },
        ...
        monitor.ingest(openLine("192.168.50.77:3000", sid: "c"))
        await sleepUntil({ self.naming.calls.count == 3 },
                         message: "the sequencing probe was never asked")
        monitor.publishNow()

        XCTAssertEqual(monitor.clients.first(where: { $0.ip == "192.168.50.42" })?.hostname,
                       "macbook-vasya",
