...
final class ProxyManagerShareTests: XCTestCase {
    ...
    func testExitIPIsFoldedIntoTheTXTRecord() async {
        let rig = makeRig(exitIP: "198.51.100.7\n")
        rig.manager.startShare()
        await rig.awaitLaunch()
        rig.controller?.started(pid: 42)
        await sleepUntil { rig.manager.lastExitIP != nil }

        XCTAssertEqual(rig.manager.lastExitIP, "198.51.100.7", "trailing newline from curl is trimmed")
        XCTAssertEqual(rig.advertiser.lastTXT?.exitIP, "198.51.100.7")
        XCTAssertEqual(proxyTXTRecord(rig.advertiser.lastTXT!)["vpnip"], "198.51.100.7")
    }
    ...
    func testExitIPProbeRetriesWhileGostIsStillBinding() async {
        ...
        let flaky = FlakyExitIPProbe(failures: 2, ip: "198.51.100.7")
        ...
        XCTAssertEqual(rig.manager.lastExitIP, "198.51.100.7")
        XCTAssertEqual(proxyTXTRecord(rig.advertiser.lastTXT!)["vpnip"], "198.51.100.7",
