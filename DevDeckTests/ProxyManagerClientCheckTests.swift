...
final class ProxyManagerClientCheckTests: XCTestCase {
    ...
    func testCheckSuccessPublishesTheMeasuredIP() async {
        let probe = RecordingProbe(response: "198.51.100.7\n")
        ...
        XCTAssertEqual(manager.clientCheck, .success("198.51.100.7"),
                       "trailing newline from curl is trimmed")
        XCTAssertEqual(probe.probedURLs, ["http://192.168.50.117:9999"],
    ...
    func testCheckShowsRunningWhileTheProbeIsInFlight() async {
        let probe = RecordingProbe(response: "198.51.100.7", blocked: true)
        let (manager, _, fake) = makeManager(probe: probe)
        await activateOpenProxy(manager, fake)
        ...
        probe.release()
        await sleepUntil { manager.clientCheck == .success("198.51.100.7") }
    }
    ...
    func testCheckSendsTheActiveProxyCredentials() async {
        ...
        manager.startDiscovery()
        fake.emit([DiscoveredProxy(name: "locked", host: "192.168.50.117", port: 8888,
        ...
        XCTAssertEqual(probe.probedURLs, ["http://dev:s3cret@192.168.50.117:8888"],
